    ...
    <TableScroll maxHeight={maxHeight}>
      <table className="w-full min-w-[880px] text-sm">
        ...
        <tbody>
          {items.length === 0 ? (
            <tr>
              <td colSpan={showFlow ? 10 : 9} className="px-3 py-10 text-center text-text-dim">
            ...
            items.map((r, i) => (
              <tr key={rowKey(r, i)} className="border-b border-border-soft hover:bg-bg-hover/40">
                ...
                {/* Name — only carried on alerts fired after it was added, so
                    older rows show a dash rather than a wrong guess. */}
                <td className="px-3 py-3">
                  <span className="text-text-muted">{r.token_name || "—"}</span>
                </td>
                <td className="px-3 py-3">
